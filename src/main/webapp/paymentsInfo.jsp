  ...
  <body>
   <div class="sidenav">
       <div class="login-main-text">
           ...
           - <fmt:message key="name.app.payments" /><br>_____________________________________________ <br></h2>
           <p> <fmt:message key="name.app.action.sort" />
               <form style="font-size: xx-small; margin: 0,0,0,0" name="formLog" action=<fmt:message key="form.actionPaymentsMenu" />  method="post">
                   <select style="font-size: small; margin: 0,0,0,0; " class="btn btn-black-option" name="selectParamSortPayments">
                       <c:forEach var="sort" items="${paramSortPaymentList}">
                           <c:if test="${selectParamSortPayments == sort}">
           ...
           <p><fmt:message key="button.makePayment" /></p>

           <form name="formLog" action=<fmt:message key="form.actionPaymentsCreateInfo" />  method="post">
               <button type="submit" class="btn btn-black"><fmt:message key="button.create" /> </button>
           </form>
           _____________________________________________ <br>
           <p><fmt:message key="button.backToMenu" /></p>

           <form name="formLog" action=<fmt:message key="form.actionLogin" />  method="post">
   ...
   <div class="main">
       ...
       <div>
           <table class= "tablePayments" style="font-family: Arial, sans-serif; width: 42%;text-align: center; margin-left: 10%; margin-top: 16%; position: fixed;  z-index: 1; font-weight: 100; padding: 60px" border = "3">
               ...
               <c:forEach var="sort" items="${payments}">
                   <tr>
                       <td> ${sort.getId()} </td>
                       <td> ${sort.getDate()} </td>
                       <td> ${sort.getSum()} </td>
