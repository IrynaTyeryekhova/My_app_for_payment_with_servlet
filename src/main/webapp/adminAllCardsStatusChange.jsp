  ...
  <body>
   <div class="sidenav">
       <div class="login-main-text">
           ...
           - <fmt:message key="name.app.cardUnlockingBlockingMenu" /><br>_____________________________________________ <br></h2>
           <p> <fmt:message key="name.app.action.sort" />
               <form style="font-size: xx-small; margin: 0,0,0,0" name="formLog" action=<fmt:message key="form.actionAdminCardsMenu.blockUnblockCards" />  method="post">
           ...
           <form name="formLog" action=<fmt:message key="form.actionAdminCardsMenu.blockUnblockCardsAnotherLang" />  method="post">
   ...
   <div class="main">
       <div >
           ...
           <form name="formLog" action=<fmt:message key="form.actionAdminCardsMenu.blockUnblockCards" />  method="post">
       ...
       <div>
           <form name="formLog" action=<fmt:message key="form.actionCardStatusChangeAdminAll" />  method="post">
               <table class= "tablePayments" style="font-family: Arial, sans-serif; width: 42%;text-align: center; margin-left: 10%; margin-top: 18%; position: fixed;  z-index: 1; font-weight: 100; padding: 60px" border = "3">
                   <tr>
                       ...
                       <th> <fmt:message key="card.info.statusForTable" /> </th>
                   </tr>

                   <c:forEach var="sort" items="${cards}">
                       <tr>
                           <td> ${sort.getNumber()} </td>
                           <td> ${sort.getBalance()} </td>
                           <td> ${sort.getValidityPeriod()} </td>
                           <td> <fmt:message key="${sort.getStatus()}" /> </td>
                           <td> <button style="width: 130px;" type="submit" class="btn btn-blacktwo" name = "cardStatusSend" value = "${sort.getNumber()}"><fmt:message key="button.statusChange" /></button></td>
                       </tr>
                   </c:forEach>
               </table>
           </form>
